...
server = app.server  # WSGI entry point — hosts like Render/Railway run this via `gunicorn dashboard:server`
storage.init_db()  # runs on import too, not just __main__ — matters for gunicorn, which never hits __main__
...
if __name__ == "__main__":
    import os
    # In production (Render/Railway/etc.), the host actually runs this file
    # via gunicorn ("gunicorn dashboard:server"), which never executes this
    # block — this branch only matters for local/Colab runs.
    debug_mode = os.environ.get("PRAVAAH_DEBUG", "false").lower() == "true"
